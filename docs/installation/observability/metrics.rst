...
Open Product produces application metrics (using Open Telemetry).
...
.. important:: Some metrics are defined as "global scope".
   ...
   .. code-block:: promql
       ...
       avg by (type) (otel_openproduct_auth_user_count{scope="global"})
...
``openproduct.auth.user_count``
    ...
    .. code-block:: promql
        ...
        max by (type) (last_over_time(
          otel_openproduct_auth_user_count{scope="global"}
          [1m]
        ))

``openproduct.auth.login_failures``
...
``openproduct.auth.user_lockouts``
