...
# Dump component data
# This script is not intended for a data migration to another Open Product instance.
...
# pg_dump also does not add related tables automatically, so `dump_data.sh producten` does not add related data from producttype to the dump.
...
export PGUSER=${DB_USER:-openproduct}
export PGDATABASE=${DB_NAME:-openproduct}
