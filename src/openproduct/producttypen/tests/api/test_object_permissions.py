...
class TestProductType(APITestCase):
    path = reverse_lazy("producttype-list")

    def setUp(self):
        self.producttype = ProductTypeFactory.create()

        self.detail_path = reverse("producttype-detail", args=[self.producttype.uuid])

        self.user = UserFactory()
        self.user.user_permissions.set(
            Permission.objects.filter(codename__contains="_producttype")
        )
    ...
    def test_view_perm(self):
        ...
        with self.subTest("with"):
            assign_perm("view_producttype", self.user, self.producttype)
    ...
    def test_update_perm(self):
        ...
        with self.subTest("with"):
            assign_perm("change_producttype", self.user, self.producttype)
    ...
    def test_delete_perm(self):
        ...
        with self.subTest("with"):
            assign_perm("delete_producttype", self.user, self.producttype)
